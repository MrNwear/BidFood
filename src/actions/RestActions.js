...
const getBrands = async dispatch => {
  try {
    ...
    // console.log('TIBU Brands : ' + JSON.stringify(data))
    dispatch({
...
export const addToCart =
  ({ cartId, item, customer, region }) =>
    async dispatch => {
      try {
        if (cartId) {
          return dispatchAddToCart(dispatch, cartId, item, region);
        }
...
const dispatchAddToCart = async (dispatch, cartId, item, region) => {
  try {
    let result;
    if (magento.isCustomerLogin()) {
        ...
        if (items != null && items != undefined) {
          ...
          const simpleItemsWithPriceAndName = simpleItems.map(simpleItem => {
            if (simpleItem.parent_item) {
              simpleItem.price = simpleItem.parent_item.extension_attributes.price_with_vat;
...
export const addAccountAddress = (id, customer, callback) => async dispatch => {
  try {
...
export const addGuestCartBillingAddress =
  ...
  export const addcustomCartBillingAddress =
...
export const getFilteredProducts =
  (category_id,minPrice,maxPrice,brand_id,availableInStock) =>
    async dispatch => {
      ...
      // axios.get(`https://dev.bidfoodhome.ae/s44/rest/V1/products/?searchCriteria[filter_groups][0][filters][0][field]=category_id&searchCriteria[filter_groups][0][filters][0][value]=${category_id}&searchCriteria[filter_groups][0][filters][0][condition_type]=eq&searchCriteria[filter_groups][1][filters][0][field]=special_price&searchCriteria[filter_groups][1][filters][0][value]=${minPrice}&searchCriteria[filter_groups][1][filters][0][condition_type]=gteq&searchCriteria[filter_groups][1][filters][1][field]=price&searchCriteria[filter_groups][1][filters][1][value]=${minPrice}&searchCriteria[filter_groups][1][filters][1][condition_type]=gteq&searchCriteria[filter_groups][2][filters][0][field]=special_price&searchCriteria[filter_groups][2][filters][0][value]=${maxPrice}&searchCriteria[filter_groups][2][filters][0][condition_type]=lteq&searchCriteria[filter_groups][2][filters][1][field]=price&searchCriteria[filter_groups][2][filters][1][value]=${maxPrice}&searchCriteria[filter_groups][2][filters][1][condition_type]=lteq&searchCriteria[filter_groups][3][filters][0][field]=product_brand&searchCriteria[filter_groups][3][filters][0][value]=${brand_id}&searchCriteria[filter_groups][3][filters][0][condition_type]=eq&searchCriteria[filter_groups][4][filters][0][field]=status&searchCriteria[filter_groups][4][filters][0][condition_type]=eq&searchCriteria[filter_groups][4][filters][0][value]=1`,{headers:{'Authorization':'Bearer wb2s1euayoz8sszqdktjxxxd8ud7jwp1'}})
      let path=`https://dev.bidfoodhome.ae/s44/rest/V1/products/?searchCriteria[filter_groups][0][filters][0][field]=category_id&searchCriteria[filter_groups][0][filters][0][value]=${category_id}&searchCriteria[filter_groups][0][filters][0][condition_type]=eq`;
      ...
      let status=`&searchCriteria[filter_groups][4][filters][0][field]=status&searchCriteria[filter_groups][4][filters][0][condition_type]=eq&searchCriteria[filter_groups][4][filters][0][value]=1`
      path+=status;
      axios.get(path,{headers:{'Authorization':'Bearer wb2s1euayoz8sszqdktjxxxd8ud7jwp1'}})
      .then(response=>{
       console.log(JSON.stringify(response.data));
       if(availableInStock){
       // alert(response.data.items[0].extension_attributes[store]);
        let avaiableItems=response.data.items.filter(item=>item.extension_attributes[store]==true);
        console.log(JSON.stringify(avaiableItems));
       // alert(avaiableItems.length);
        let temp={...response.data,items:avaiableItems,total_count:avaiableItems.length};
       
        dispatch({ type: MAGENTO_GET_FILTERED_PRODUCTS, payload:  temp});
       }
       else{

         dispatch({ type: MAGENTO_GET_FILTERED_PRODUCTS, payload: response.data });
       }
