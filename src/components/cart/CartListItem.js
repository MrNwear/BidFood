...
const CartListItem = ({
  ...
  const DiscountParentage = () => {
    if (products && products[item.sku]) {
      let basePrice = products[item.sku].extension_attributes.price_with_vat;
      let discountPrice = finalPrice(
        products[item.sku].custom_attributes,
        basePrice,
      );
  ...
  const renderPrice = () => {
    if (products && products[item.sku]) {
      ...
      if (!finalPrice(products[item.sku].custom_attributes, basePrice)) {
        return (
          <Price
            style={{
              fontSize: 16,
