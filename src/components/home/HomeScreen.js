...
import { getHomeData, setCurrentProduct, getCategoryTree, resetFilters, setCurrentCategory } from '../../actions';
...
import flash_sales from '../../assets/flash_sales.jpeg';
...
class HomeScreen extends Component {
  ...
  componentDidMount() {
    ...
    // }
    let category_idList=[];
    axios.get('https://dev.bidfoodhome.ae/rest/all/V1/cmsBlock/11',{headers:{'Authorization':'Bearer wb2s1euayoz8sszqdktjxxxd8ud7jwp1'}}).then(response=>{
      let {content}=response.data;
      let temp=content.slice(content.search('category_id')+13,content.indexOf('"',content.search('category_id')+14));
       category_idList=  temp.split(',');
       for(let i=0;i<category_idList.length;i++){
        
        axios.get('https://dev.bidfoodhome.ae/rest/V1/categories/'+category_idList[i],{headers:{'Authorization':'Bearer wb2s1euayoz8sszqdktjxxxd8ud7jwp1'}}).then(response=>{
        this.setState(prevState=>{return{categories:[...prevState.categories,response.data]}});
        console.log(this.state.categories);
    }
      )
  }
    }).catch(error=>console.log(error));
    axios.get('https://dev.bidfoodhome.ae/rest/all/V1/cmsBlock/44',{headers:{'Authorization':'Bearer wb2s1euayoz8sszqdktjxxxd8ud7jwp1'}}).then(response=>{
      
      //this.setState(prevState=>{return{products:[...prevState.products,response.data]}});
      let {content}=response.data;
      let temp=content.slice(content.search('FlashSales')+14,content.indexOf('"',content.search('FlashSales')+15));
       category_idList=  temp.split(',');
       console.log(category_idList);
       for(let i=0;i<category_idList.length;i++){

         axios.get('https://dev.bidfoodhome.ae/rest/V1/products/'+category_idList[i],{headers:{'Authorization':'Bearer wb2s1euayoz8sszqdktjxxxd8ud7jwp1'}}).then(res=>{
           this.setState(prevState=>{return{bestSellers:[...prevState.bestSellers,res.data]}})
          // console.log('best selllerssf ssssssss',res.data);
          })
        }
         temp=content.slice(content.search('BestSellers')+15,content.indexOf('"',content.search('BestSellers')+16));
        category_idList=  temp.split(',');
        for(let i=0;i<category_idList.length;i++){

          axios.get('https://dev.bidfoodhome.ae/rest/V1/products/'+category_idList[i],{headers:{'Authorization':'Bearer wb2s1euayoz8sszqdktjxxxd8ud7jwp1'}}).then(res=>{
            this.setState(prevState=>{return{newProducts:[...prevState.newProducts,res.data]}})
           // console.log('best selllerssf ssssssss',res.data);
           })
         }
  //     let temp=content.slice(content.search('category_id')+13,content.indexOf('"',content.search('category_id')+14));
      ...
      <>
        ...
        <ScrollView
          ...
          <View style={styles.Slider}>
            <HomeSlider key={"homeSlider"} slider={this.props.slider} setCurrentProduct={this.props.setCurrentProduct} />
          </View>
          <FeaturedCategories key={"featuredCategories"} homeCategories={this.state.categories} />
