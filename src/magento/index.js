...
class Magento {
  ...
  getHomeData() {
    if (this.configuration.home_cms_block_id) {
      ...
      console.log("this.configuration.home_cms_block_id", this.configuration.home_cms_block_id);

      let cmsBlock = this.admin.getCmsBlock(this.configuration.home_cms_block_id);
  ...
  guestOrderRecord = async (DEMO_TOKEN, ORDERID) => {
    try {
      ...
      const response = await fetch(`${truncatedUrl}/rest/s44/V1/orders/${ORDERID}`, {
        method: "GET",
        headers: {
